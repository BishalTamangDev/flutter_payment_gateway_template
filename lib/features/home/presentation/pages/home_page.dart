...
class _HomePageState extends State<HomePage> {
  // variables
  late Future<Either<Failure, List<ProductModel>>> _products;

  // functions
  // fetch products
  Future<Either<Failure, List<ProductModel>>> _fetchProducts() async {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: SingleChildScrollView(
          child: FutureBuilder<Either<Failure, List<ProductModel>>>(
            future: _products,
            builder: (context, snapshot) {
              if (snapshot.connectionState == ConnectionState.done) {
                if (snapshot.hasError) {
                  ...
                  final Either<Failure, List<ProductModel>>? result = snapshot.data;
                  return result!.fold((failure) => Center(child: Text(failure.message)), (data) {
